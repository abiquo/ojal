...
func newDatacenterRepository() Resource { return new(DatacenterRepository) }

// Templates returns the Datacenter Repository Virtual Machine templates collection
func (d *DatacenterRepository) VirtualMachineTemplates(query url.Values) *Collection {
...
func (d *DatacenterRepository) Upload(file string) (v *VirtualMachineTemplate, err error) {
	...
	reply, err := Upload(endpoint, file)
	if err == nil {
